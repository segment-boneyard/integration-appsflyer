...
AppsFlyer.prototype.track = function(track, fn) {
  ...
  var options = this.options('AppsFlyer');
  var device = track.device() || {};
  var revenue = track.revenue(); // will use properties.total for `order completed` events
  var timestamp = track.timestamp().toISOString().replace(/T/, ' ').replace(/\..+/, ''); // needs to be in UTC timezone
  var payload = {
  ...
  revenue ? payload.eventValue.revenue = revenue : payload.eventValue = '';

  if (contains(track.library(), 'ios')) {
    payload.idfa = device.advertisingId;
    payload.bundle_id = track.proxy('context.app.namespace');
  } else {
    payload.advertising_id = device.advertisingId;
  }
...
function selectEndpoint(library, settings) {
  ...
  if (contains(library, 'ios')) {
    endpoint = settings.appleAppID;
  } else {
    endpoint = settings.appsFlyerDevKey;
  }
  return endpoint;
}
